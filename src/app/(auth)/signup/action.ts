...
export async function signupAction(formData: SignupCombineData) {
  ...
  try {
      console.log('process.env.NEXT_PUBLIC_BACKEND_URL:', process.env.NEXT_PUBLIC_PROD_BACKEND_URL+'/auth/signup')
    const res = await fetch(`${process.env.NEXT_PUBLIC_PROD_BACKEND_URL}/auth/signup`, {
      method: "POST",
